...
from contracts.enums import DecisionMode, FreshnessStatus, Market
from contracts.execution import (CurrentOrderPreview, ExecutionEvidenceGrade, ExecutionPolicy, ExecutionState, OrderIntent, PreviewStatus)
from contracts.market_data import ContractViolation, ensure_utc, stable_hash
from contracts.risk import MarketRuleSet, MarketState
...
class CurrentPreviewBuilder:
    ...
    def build(self, intent: OrderIntent, state: ExecutionState, market_state: MarketState | None, rules: MarketRuleSet, liquidity, observed_at: datetime) -> CurrentOrderPreview:
        ...
        if observed_at >= intent.expires_at:
            ...
            from contracts.execution import ExecutionEvent, EventGranularity, TradingStatus
