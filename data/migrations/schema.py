...
# V2-12 后续兼容修复；旧版本 SQL/checksum 保持不可变。
SCHEMA_VERSION = 19
...
# V2-12 初版的 legacy executor 将数据库迁移号 17 误当成
# WatchlistSnapshot 的对象合同版本。该合同版本固定为 1；只修正已知错误值，
# 不影响未来可能出现的其他对象版本。
_SCHEMA_V18_SQL = """
...
_SCHEMA_V19_SQL = """
CREATE TABLE IF NOT EXISTS forecast_validation_summaries (
    ...
    data_hash TEXT NOT NULL,
    created_at TEXT NOT NULL,
    UNIQUE(market, scope_key, horizon, data_hash)
);
CREATE INDEX IF NOT EXISTS idx_forecast_validation_latest
ON forecast_validation_summaries(market, scope_key, horizon, created_at DESC);
CREATE TABLE IF NOT EXISTS forecast_candidate_evaluations (
    ...
    payload_json TEXT NOT NULL,
    created_at TEXT NOT NULL,
    UNIQUE(market, scope, scope_key, horizon, spec_id, phase, data_hash)
);
CREATE INDEX IF NOT EXISTS idx_forecast_candidate_evaluation_lookup
ON forecast_candidate_evaluations(market, scope_key, horizon, created_at DESC);
""".strip()
...
def schema_v18_checksum() -> str:
    return sha256(_SCHEMA_V18_SQL.encode("utf-8")).hexdigest()

def schema_v19_checksum() -> str:
    return sha256(_SCHEMA_V19_SQL.encode("utf-8")).hexdigest()
...
def apply_schema(connection: sqlite3.Connection) -> None:
    ...
    _apply_migration(connection, 18, _SCHEMA_V18_SQL, schema_v18_checksum())
    _apply_migration(connection, 19, _SCHEMA_V19_SQL, schema_v19_checksum())
    connection.commit()
