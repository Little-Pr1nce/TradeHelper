...
from contracts.enums import ProviderStatus, TradingSession
...
from contracts.providers import ProviderResult, QuoteBatch
...
class TickFlowAdapter:
    ...
    def quote(self, instrument: InstrumentId, session: TradingSession) -> ProviderResult[QuoteSnapshot]:
        def parse(raw) -> QuoteSnapshot:
            rows = tuple(raw) if isinstance(raw, (list, tuple)) else (raw,)
            if not rows or not isinstance(rows[0], Mapping):
                raise ValueError("TickFlow quote response has no quote row")
            return parse_tickflow_quote(rows[0], instrument, session, self.now())

        return _result(
            self.name,
            self.now,
            lambda: self.quote_call([self.symbol(instrument)]),
            parse,
        )
