...
from .research import HypothesisOutcome, HypothesisValidation, ResearchHypothesis, ResearchMetricSnapshot, ResearchRunStatus
...
""".split())
PRESENTATION_POLICY_REF = "external:tradehelper_presentation_policy_v1"
...
class PortfolioPresentationInput:
    ...
    portfolio_research_hypotheses: tuple[ResearchHypothesis, ...]
    portfolio_research_validations: tuple[HypothesisValidation, ...]
    research_status: ResearchRunStatus
    research_chunk_count: int
    research_completed_chunk_count: int
    research_failure_reasons: tuple[str, ...]
    source_artifact_refs: tuple[str, ...]
    built_at: datetime
    ...
    def __post_init__(self):
        ...
        research_status = _enum(ResearchRunStatus, self.research_status, "portfolio research status")
        research_failures = tuple(sorted(set(self.research_failure_reasons)))
        if (isinstance(self.research_chunk_count, bool) or isinstance(self.research_completed_chunk_count, bool) or
                self.research_chunk_count < 0 or not 0 <= self.research_completed_chunk_count <= self.research_chunk_count or
                any(not isinstance(item, str) or not item for item in research_failures)):
            raise ContractViolation("invalid portfolio research run summary")
        portfolio_hypotheses = tuple(sorted(self.portfolio_research_hypotheses, key=lambda item: item.hypothesis_id))
        portfolio_validations = tuple(sorted(self.portfolio_research_validations, key=lambda item: item.validation_id))
        refs = _refs(self.source_artifact_refs)
        ...
        if (self.account_snapshot.captured_at > as_of or
                ...
                any(item.generated_at > built for item in (
                    ...
                    *portfolio_hypotheses, *portfolio_validations,
                ))):
        ...
        identity = {
            ...
            "research": self.portfolio_research_evidence,
            "research_hypotheses": portfolio_hypotheses,
            "research_validations": portfolio_validations,
            "research_status": research_status,
            "research_chunk_count": self.research_chunk_count,
            "research_completed_chunk_count": self.research_completed_chunk_count,
            "research_failure_reasons": research_failures,
            "refs": refs,
        }
        if self.presentation_id != stable_hash(identity):
        ...
        for name, value in (
            ...
            ("portfolio_research_hypotheses", portfolio_hypotheses),
            ("portfolio_research_validations", portfolio_validations),
            ("research_status", research_status),
            ("research_failure_reasons", research_failures),
