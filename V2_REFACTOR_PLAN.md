...
## 32. 2026-08-06 桌面启动与关闭生命周期修复

- Runtime 生命周期从 Flet `Page` 会话提升到桌面进程：同一进程发生页面重连或会话重建时复用唯一 repository、线程池和业务容器，不再重复初始化；窗口关闭并让 `ft.run()` 返回后，进程级 `finally` 只执行一次资源回收。
- 删除 `page.on_close` 中同步关闭线程池和数据库的做法。原实现会让窗口关闭事件承担后台任务等待，并混淆页面会话与原生窗口边界；现在原生窗口先正常关闭，随后进程收尾，macOS 实测单击一次关闭按钮即以退出码 0 结束。
- 桌面壳只构建首屏“单股分析”，我的持仓、能力评估、报告记录、设置和迁移页在首次点击时按需构建。报告历史列表改为读取冻结快照摘要，不再逐份反序列化、哈希并校验整篇报告；完整校验仍在用户打开具体报告时执行。
- 当前真实数据库含 39 份报告：历史列表由约 `8.8s` 降至约 `0.9s`，首屏控件构建由约 `8.9s` 降至 `0.002s`。真实 Flet 窗口日志记录会话构建 `0.022s`、关闭资源回收 `0.001s`，且启动与关闭均只出现一次。
- V2 全量回归为 `908 passed, 4 skipped in 83.50s`；4 项仍为默认关闭的真实 Provider/LLM 联网冒烟，与本轮生命周期修改无关。

## 33. 2026-08-06 建议可信度与可执行性收紧

- 对真实已到期预测复核发现：仅有 OOF 晋升不足以保证当前仍可执行。新增市场 × 周期在线表现门槛，使用去重后的最近 120 个成熟事件比较方向正确率、多数类基准和 Brier；样本不足不武断否决，明显低于简单基准时保留概率展示但暂停影响新开仓。
- 在线暂停形成独立 `live-observation-v1` 预测身份，避免同一交易日重复分析与原预测事件发生数据库冲突；正式模型的 OOF 指标继续按候选侧展示，不伪装成经验基线。
- 策略政策升级为 `strategy_policy_v2`。等待中的买入/加仓触发价若超出有效期内由 ATR、1 日预测区间和绝对上限共同确定的合理可达范围，统一降级为远期观察，不进入风控批准和组合资金预留。SNDK 一类“现价约 1350、等待约 1958 才买”的计划因此不再作为可执行买入建议。
- 报告把“现在做什么”和“未来达到什么条件再做”彻底分开：等待入场不再归入当前买入，等待退出不再冒充立即卖出；非当前可执行计划不展示可下单股数。
- 保守与激进分别按自身风险档案和精确 `plan_id` 读取组合分配，修复持有计划借用其他卖出计划股数、两种方案被错误显示为相同等问题。
- 技术面和成交量确认改为系统自动监测状态，用户无需自行计算“放量多少”；报告按价格条件、系统确认、数据要求、执行方式和有效期分行说明，并展示已知当前值。
- 新增在线预测质量、重复事件去重、双市场账本隔离、远距触发降级、当前动作映射、计划分配隔离及保守/激进独立映射测试。最终全项目回归为 `918 passed, 4 skipped in 78.45s`；4 项仍为默认关闭的真实 Provider/LLM 联网冒烟。

## 34. 2026-08-06 股票级预测模型真实 OOF 强化

- 修复模型生命周期硬伤：新一轮同股票、同周期 OOF 明确失败时，旧 Champion 不再永久留在数据库和内存 registry；repository 以可选 `expected_version` 防止迟到后台任务误退役刚晋升的新版本。样本暂时不足不触发退役。
- 受控候选池保持 20 个上限，加入固定 120/250 日经验窗口，以及同市场面板的 ExtraTrees 和 logistic 候选。面板模型只共享方向关系；校准、收益区间、confirmation 和 Champion 仍绑定目标股票，A股与美股不混池。ExtraTrees 固定 24 棵浅树、固定随机种子、`n_jobs=1`，artifact 继续只保存 JSON 节点数组并 zlib 压缩，不引入 pickle/joblib。
- OOF 继续使用完整扩张历史拟合；独立开发回放比较 120/252/360 日 selection 后，冻结为 confirmation 之前最近 360 个完整交易日。confirmation 仍完全隔离，禁止事后改选在盲测段表现最好的候选。
- 概率校准由单一温度缩放升级为“温度 + 训练段类别先验收缩”。校准段只选择收缩强度，收缩目标只能来自更早训练段；随机序列不得因近期类别频率偶然偏斜被误晋升。
- empirical 方向概率不再被温度校准重复扭曲，内部校准段只调整收益区间宽度；market-panel 候选也只能使用目标股票的校准段和收益历史。所有跨股票训练点在每个 OOF 原点都重新执行标签成熟检查，避免面板未来泄漏。
- 当复杂候选没有跑出可信增量时，新增独立的 `validated-empirical-stock` 绝对质量门。它要求 selection 与 confirmation 同时满足样本量、Brier、Log Loss、ECE 和区间覆盖护栏，只能成为 `noninferior` B 级基线，不能宣称发现 Alpha，也没有放宽原候选晋升门槛。
- 真实缓存日 K 对 10 只美股、4 个周期、每组最多 720 个 OOF 起点和 20 个候选进行了 10 路并行只读验证，共 40 组。M5 10 核实测墙钟 `139.44s`、用户态 CPU `1208.48s`、内核态 `12.71s`，平均约使用 `8.76` 个核心。
- 最终同口径真实回放为 `1 confirmation_passed / 28 noninferior_passed / 11 failed`，正式可用模型覆盖由上一轮 `15/40` 提升到 `29/40`（72.5%）：1日 9/10、3日 9/10、5日 6/10、10日 5/10。11 个失败组合继续明确保留 `calibration_failed` 或 `evaluated_not_better`，没有用 confirmation 反向选模或降低门槛。
- `29/40` 是通过独立 OOF 质量门的模型覆盖率，不是方向正确率，更不等于 72.5% 盈利概率。严格优于基线的只有 1 组，其余 28 组只能提供 B 级小仓证据；真实方向命中、收益和回撤仍由到期预测账与联合策略账分别统计。
- 当前生产数据库只有美股 canonical 日 K，没有可用于真实 A 股 OOF 的本地样本，因此本轮不能伪称已完成 A 股真实训练对照。A股合同、市场隔离和算法路径继续由自动化测试覆盖；下一次 A 股真实分析积累 canonical 日 K 后，必须用同一基准脚本补做 600519 等代表股票验证。
- 新增市场面板无泄漏、个股校准、经验概率保持、固定窗口、后台面板训练和独立基线质量门测试；本轮全项目回归为 `931 passed, 4 skipped`。4 项仍是默认关闭的真实 Provider/LLM 联网冒烟，与预测算法修改无关。
