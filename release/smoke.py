"""离线发布冒烟：验证生产 composition、当前 schema 和报告 renderer。"""
...
from contracts import DecisionMode, Exchange, InstrumentId, Market, NewsSnapshot, ReportBlock, ReportBlockKind, ReportDocument, ReportKind, ReportSection, stable_hash
...
from release.manifest import verify_manifest
...
def run_smoke(work_dir: Path | str | None = None) -> dict[str, object]:
    ...
    try:
        ...
        manifest_path=Path(__file__).resolve().parents[1]/"dist_data"/"release-manifest.json"
        ...
        return {"ok":True,"schema_version":SCHEMA_VERSION,"html":len(html),"markdown":len(markdown),"pdf":len(pdf),"finbert_label":getattr(finbert_result,"finbert_label",None),"manifest":manifest,"manifest_reasons":manifest_reasons,"health":reopened.health()}
