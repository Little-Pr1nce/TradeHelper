...
8. **综合建议与短期预测（AI分析）** — 这是报告中最重要的部分。你必须交叉印证上面所有数据（因子得分、IC/IR 检验、基本面估值、技术指标、新闻情绪、回测绩效、Rank IC、基准收益、**盘口买卖比**、**实时报价**等），给出：
   a) **数据综合分析**：从多个维度交叉验证当前市场状态（例如因子 IC 有效 + 新闻偏多 + 策略回测盈利 → 信号一致性强；因子 IC 为负 + 策略跑输基准 → 模型当前预测力不足，需谨慎）。**盘口数据反映了当下买卖力量对比**（买盘占优/卖盘占优/基本平衡），**实时报价显示了当前涨跌和市场情绪**，这些必须被纳入分析。
   b) **操作建议**：基于数据给出短期方向判断和关键参考点位。**综合盘口买卖比、实时涨跌幅、Alpha 因子方向来给出更精准的判断**。
   c) **短期走势预测**：对未来 1-4 周走势做判断，并说明置信度（高/中/低）及依据。**盘口买卖力量对比是短期方向的重要参考**，结合技术面和新闻面综合预判。如果盘口数据显示买盘显著占优且实时报价上涨，短期偏多概率更高。
"""
