...
2. **严禁编造数据** — 所有分析必须基于我提供的数据，不得添加未提及的数字或事实。尤其注意：**不要编造你没有数据的日期的事件**（如今天的跳空缺口、开盘表现等）。如果你不知道今天开盘后发生了什么，就不要讨论今天的盘中走势和缺口。你能讨论的只有数据中已有的信息。
...
6. **模糊术语必须量化** — 禁止只用「企稳」「放量」「缩量」「阳线」等模糊词。每次使用必须注明你基于什么数值做出的判断。具体阈值由你根据该股票的实际数据特征来定，不要照搬固定数字。比如不能只说「放量阳线企稳」，要说「成交量较前 5 日均量放大约 40%，当日涨幅 +2.3%，且价格连续 3 日未创新低，判断此处企稳」。
7. **同板块必须独立成章** — 如果用户 prompt 中提供了「同板块 Alpha 快速评分」数据，你必须输出独立的「## 九、同板块关注」章节（含 Markdown 表格逐条展示所有标的），**不得**将同板块内容合并到第十章综合建议中。就算同板块标的很少，也要单独成章。
...
10. **综合建议与短期预测（AI分析）** — 这是报告中最重要的部分。你必须交叉印证上面所有数据（因子得分、IC/IR 检验、基本面估值、SWOT 分析、技术指标、新闻情绪、回测绩效、同板块对比、Rank IC、基准收益、**盘口买卖比**、**实时报价**等），给出：
   ...
   8.2) **操作建议**：这是报告中最具可执行性的部分，必须详细、具体、可操作。请按以下格式输出：
      ...
      **分批操作计划**（这是整份报告的最终操作结论，必须综合前面所有分析得出）：
      ...
      分批操作计划以「**综合操作方案**」为标题，先写一段 **「决策依据」**（3-5 句话，说明综合了哪些信息、为什么得出以下方案、哪些因素最重要），再给具体计划：
...
6. **模糊术语必须量化** — 禁止只用「企稳」「放量」「缩量」「阳线」等模糊词。每次使用必须注明基于什么数值判断，阈值根据该股票实际数据来定，不照搬固定数字。
7. **盘中数据时效声明** — 所有点位基于快照时刻的实时价格计算，会随行情变化。
...
**🎯 策略操作转化**（必须输出）：从量化策略和人类策略中各选最优的，共输出 2 个方案。量化策略→翻译成人能执行的条件+简化代价；人类策略→直接告诉用户当前如何执行（等待/买入/持有/卖出，具体价位、仓位、止损、卖出条件）。
...
5. **每个结论都要有依据** — 对于每一个判断，必须说明是基于哪个数据的什么值推导出来的，不要只给结论。
6. **模糊术语必须量化** — 禁止只用「企稳」「放量」「缩量」「阳线」等模糊词。每次使用必须注明基于什么数值判断，阈值根据该股票实际数据来定，不照搬固定数字。
7. **盘前数据时效声明** — 所有分析基于盘前数据，开盘后可能因流动性变化而改变。
