...
6. **新闻面分析** — 基于新闻情感分析结果，评估市场情绪
7. **策略回测结果** — 三种交易策略的横向对比，包括各策略的收益率、夏普比率、最大回撤等核心指标
8. **综合建议与短期预测（AI分析）** — 这是报告中最重要的部分。你必须交叉印证上面所有数据（因子得分、IC/IR 检验、基本面估值、技术指标、新闻情绪、回测绩效、Rank IC、基准收益、盘口等），给出：
   a) **数据综合分析**：从多个维度交叉验证当前市场状态（例如因子 IC 有效 + 新闻偏多 + 策略回测盈利 → 信号一致性强；因子 IC 为负 + 策略跑输基准 → 模型当前预测力不足，需谨慎）
   b) **操作建议**：基于数据给出短期方向判断和关键参考点位
   c) **短期走势预测**：对未来 1-4 周走势做判断，并说明置信度（高/中/低）及依据
"""
