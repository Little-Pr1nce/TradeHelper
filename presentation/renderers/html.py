...
def render_html(document):
    ...
    *{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Microsoft YaHei',sans-serif;margin:0;color:var(--ink);background:#fff;line-height:1.55;letter-spacing:0}
    ...
    return (
        ...
        f'<title>{escape(document.title)}</title><style>{style}</style></head><body><main><header><h1>{escape(document.title)}</h1><p>{escape(document.summary)}</p><p class="meta">数据时点：{escape(format_datetime(document.as_of, document.market, seconds=True))}</p></header>{"".join(sections)}</main></body></html>'
    )
