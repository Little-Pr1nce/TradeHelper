        ...
        if protective_spec is not None:
            ...
            if context.cost is not None and context.cost > 0:
                ...
                if ma60 is not None:
                    ...
                    level_evidence = ("closed.ma_60",)
        if candidates:
            protective_line = candidates[0]
            condition = compare(feature("current.price"), ConditionOperator.GT, level("hold_above_protection", protective_line, *level_evidence), "PLAN_WAITING")
            invalidation = compare(feature("current.price"), ConditionOperator.LTE, level("hold_invalidation", protective_line, *level_evidence), "PROTECTIVE_EXIT_PENDING")
            evidence = ("current.price",) + level_evidence
        else:
