...
from tradehelper_v2.contracts import ConditionOperator, ExitPosture, PlanAction, ScenarioState, TakeProfitMode
from .common import Proposal, all_of, always_false, compare, constant, crossing, feature, level, parameter


def protective_exit(ctx, spec):
    candidates = []
    if ctx.cost and ctx.cost > 0:
        candidates.append(ctx.cost * (1 - parameter(spec, "cost_stop_pct")))
    ma60 = ctx.number("closed.ma_60")
    if ma60:
        candidates.append(ma60 * (1 - parameter(spec, "ma60_buffer")))
    if not candidates:
        return None
    stop = max(candidates)
    triggered = ctx.pobs is not None and ctx.pobs <= stop
    code = "PROTECTIVE_EXIT_TRIGGERED" if triggered else "PROTECTIVE_EXIT_PENDING"
    return Proposal(PlanAction.SELL, compare(feature("current.price"), ConditionOperator.LTE, level("protective_stop", stop, "closed.ma_60"), code), None, stop, "protective_stop_v1", None, None, None, TakeProfitMode.NONE, None, always_false(), None, (code,), ("closed.ma_60",))
...
def profit_lock(ctx, spec):
    ...
    enabled = ctx.pobs / ctx.cost - 1 >= minimum_profit
    price_condition = compare(feature("current.price"), ConditionOperator.GTE, constant(ctx.cost * (1 + minimum_profit)), "PROFIT_LOCK_PENDING")
    ...
    condition = all_of(price_condition, retreat_condition, reason="PROFIT_LOCK_PENDING")
    evidence = ("closed.atr_pct_14", retreat_feature)
    return Proposal(PlanAction.REDUCE, condition, None, peak * (1 - threshold), "profit_lock_level_v1", None, None, None, TakeProfitMode.NONE, None, always_false(), None, (("PROFIT_LOCK_TRIGGERED" if enabled and retreat <= -threshold else "PROFIT_LOCK_PENDING"),), evidence)
