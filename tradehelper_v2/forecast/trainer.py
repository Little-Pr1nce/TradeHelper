...
def default_candidate_specs() -> tuple[ModelSpec, ...]:
    ...
    rank = 1
    for feature_set in ("tech", "full"):
        for k in (40, 80):
            specs.append(ModelSpec(f"analog-{feature_set}-k{k}", ModelFamily.ANALOG, feature_set, {"k": k}, complexity_rank=rank)); rank += 1
    for feature_set in ("tech", "tech_news", "tech_fund", "full"):
        for c in (0.1, 1.0):
            specs.append(ModelSpec(f"logistic-{feature_set}-c{c}", ModelFamily.MULTINOMIAL_LOGISTIC, feature_set, {"C": c}, complexity_rank=rank)); rank += 1
    for feature_set in ("tech", "full"):
        for depth in (2, 3):
            specs.append(ModelSpec(f"tree-{feature_set}-d{depth}", ModelFamily.PROBABILITY_TREE, feature_set, {"max_depth": depth, "min_samples_leaf": "max(15,2pct)"}, complexity_rank=rank)); rank += 1
    for feature_set in ("tech", "full"):
        specs.append(ModelSpec(f"ensemble-{feature_set}", ModelFamily.ENSEMBLE, feature_set, {"weight": 0.5}, complexity_rank=rank)); rank += 1
    for k in (40, 80):
        specs.append(ModelSpec(f"regime-analog-tech-k{k}", ModelFamily.REGIME_ANALOG, "tech", {"k": k}, complexity_rank=rank)); rank += 1
    return tuple(specs)
...
class ForecastTrainer:
    ...
    def evaluate(
        ...
        usable_specs = tuple(spec for spec in self.candidate_specs if spec.feature_set_id == "tech" or extension_coverage(data, spec.feature_set_id) >= 0.60)
        if not usable_specs:
        ...
        for spec in usable_specs:
            ...
            status = ValidationStatus.SELECTION_PASSED if self._selection_passes(selection, baseline_selection, baseline_records[:split], result[:split], horizon, scope_key, spec.spec_id, data_hash) else self._failure_status(selection, baseline_selection)
        ...
        winner, selected = min(passers, key=lambda item: (item[1].selection.multiclass_brier, item[1].selection.log_loss, item[1].selection.expected_calibration_error, item[0].complexity_rank, item[0].spec_id))
        if not self._confirmation_passes(selected.confirmation, baseline_confirmation, baseline_records[split:], records[winner.spec_id][split:], horizon, scope_key, winner.spec_id, data_hash):
        ...
        version = ForecastModelVersion(
            ...
            validation_status=ValidationStatus.CONFIRMATION_PASSED, training_start=data[0].origin_session_date,
        ...
        revised = tuple(CandidateEvaluation(item.spec, ValidationStatus.CONFIRMATION_PASSED if item.spec == winner else item.status, item.selection, item.confirmation, item.baseline_selection, item.baseline_confirmation, item.oof_count, final_model.temperature if item.spec == winner else item.temperature) for item in evaluations)
        return TrainingOutcome(scope, scope_key, horizon, ValidationStatus.CONFIRMATION_PASSED, revised, version, final_model, None)
