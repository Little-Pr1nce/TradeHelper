...
from tradehelper_v2.contracts import (CanonicalBar, CorrelationPair, CorrelationStatus, ExecutionLevel, HoldingRiskSnapshot, HoldingRiskStatus, InstrumentId, InstrumentReturnRisk, Market, PortfolioCorrelationSnapshot, PortfolioEvidenceGrade, PortfolioHeatStatus, PortfolioPolicy, PortfolioRiskSnapshot, QuantityIntent, RiskDecisionBundle, PlanAction, ValuationStatus, stable_hash)
from tradehelper_v2.risk.sizing import friction_reserve
...
def build_portfolio_risk_snapshot(*, valuation, holding_risks, correlation_snapshot, policy, calculated_at: datetime) -> PortfolioRiskSnapshot:
    ...
    weights=tuple((item.instrument,item.market_value/equity) for item in positions) if equity else ()
