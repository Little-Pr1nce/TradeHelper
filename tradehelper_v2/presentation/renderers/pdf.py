...
def render_pdf(document):
    ...
    styles = _styles()
    story = [_paragraph(document.title, styles["title"]), _paragraph(document.summary, styles["callout"]), _paragraph(f"数据时点：{document.as_of.isoformat()}", styles["small"]), Spacer(1, 5)]
