...
from tradehelper_v2.contracts.enums import DecisionMode, FreshnessStatus, Market, ProviderStatus, QualityStatus, TradingSession
...
from tradehelper_v2.contracts.providers import DailyBarsBatchResult, DailyBarsRequest, ProviderAttempt, QuoteBatch
        ...
        *,
        ...
        def load() -> ProviderResult[tuple[CanonicalBar, ...]]:
            if instrument.market is Market.US:
                primary = self._call_daily(self.providers.nasdaq_daily, instrument, fetch_start, requested_end, now)
                result = primary
                if primary.status is not ProviderStatus.OK:
                    yfinance = self._call_daily(self.providers.yfinance_daily, instrument, fetch_start, requested_end, now)
                ...
                if result.status is not ProviderStatus.OK:
                    tickflow = self._call_daily(self.providers.tickflow_daily, instrument, fetch_start, requested_end, now)
                ...
                result = (
                    ...
                    else self._call_daily(self.providers.tickflow_daily, instrument, fetch_start, requested_end, now)
                )
            ...
            response_bars = tuple(
                bar for bar in result.value if bar.trading_date <= min(requested_end, completed_end)
            )
            ...
            try:
                ...
                return ProviderResult.failure(ProviderStatus.UNAVAILABLE, now, result.attempts, "exchange calendar unavailable")
            new_bars = tuple(bar for bar in candidate_bars if bar not in invalid_sessions)
