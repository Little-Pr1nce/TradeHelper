...
def settings_page(settings, market=Market.US, mode="eod", *, save_port=None, live_test_port=None):
    ...
    def current_mapping():
        ...
        return {
            ...
            "news_token_us": original.news_token_us,
            "news_token_a": original.news_token_a,
            "finbert_model_path": original.finbert_model_path,
            "llm_enable_thinking": original.llm_enable_thinking,
        }
    ...
    def save(_event=None):
        try:
            ...
            llm_key.value = us_token.value = a_token.value = ""
            message.value = "设置已保存。工作目录已修改，需要重启程序后生效。" if changed_directory else "设置已保存。"
            message.color = ft.Colors.ORANGE_800 if changed_directory else ft.Colors.GREEN_700
            refresh_capability()
