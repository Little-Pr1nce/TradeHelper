...
from tradehelper_v2.contracts import DecisionMode, Market, ReportHistoryQuery, ReportKind
...
class ReportHistoryPage:
    ...
    def _archive(self, report_id, archived):
        def handler(_event=None):
            self.service.archive(report_id, archived=archived); self.load(); self._update()
        return handler
    ...
    def _list_content(self):
        market = ft.Dropdown(label="市场", value=self.query.market.value if self.query.market else "all", options=[ft.dropdown.Option("all", "全部"), ft.dropdown.Option("US", "美股"), ft.dropdown.Option("A", "A股")], width=140)
        kind = ft.Dropdown(label="报告类型", value=self.query.report_kind.value if self.query.report_kind else "all", options=[ft.dropdown.Option("all", "全部"), ft.dropdown.Option("single_stock", "单股"), ft.dropdown.Option("portfolio", "组合")], width=150)
        instrument = ft.TextField(label="股票代码或公司名", value=self.query.instrument.code if self.query.instrument else "", width=190)
        mode = ft.Dropdown(label="模式", value=self.query.analysis_mode.value if self.query.analysis_mode else "all", options=[ft.dropdown.Option("all", "全部"), ft.dropdown.Option("pre", "盘前"), ft.dropdown.Option("intraday", "盘中"), ft.dropdown.Option("eod", "盘后")], width=125)
        period = ft.Dropdown(label="周期", value=self.query.history_period or "all", options=[ft.dropdown.Option("all", "全部"), ft.dropdown.Option("1m", "1个月"), ft.dropdown.Option("3m", "3个月"), ft.dropdown.Option("6m", "6个月"), ft.dropdown.Option("1y", "1年")], width=125)
        date_from = ft.TextField(label="起始日期 YYYY-MM-DD", value=self.query.date_from.date().isoformat() if self.query.date_from else "", width=175)
        date_to = ft.TextField(label="结束日期 YYYY-MM-DD", value=self.query.date_to.date().isoformat() if self.query.date_to else "", width=175)
        rating = ft.Dropdown(label="最低评分", value=str(self.query.minimum_rating) if self.query.minimum_rating else "all", options=[ft.dropdown.Option("all", "全部"), *(ft.dropdown.Option(str(item), f"{item} 星及以上") for item in range(1, 6))], width=135)
        ...
        controls = [
            ft.Text("历史报告", theme_style=ft.TextThemeStyle.HEADLINE_SMALL),
            ft.Row([market, kind, instrument, mode, period, archived], wrap=True),
            ft.Row([date_from, date_to, rating, ft.IconButton(ft.Icons.FILTER_ALT, tooltip="应用筛选", on_click=self._filter(market, kind, instrument, mode, period, date_from, date_to, rating, archived)), ft.Button("比较", icon=ft.Icons.COMPARE_ARROWS, on_click=self._compare, disabled=not self.selected)], wrap=True),
        ]
        if self.error: controls.append(ft.Text(self.error, color=ft.Colors.RED_700 if "最多" in self.error else ft.Colors.BLUE_700))
        controls.append(ft.Text(f"共 {0 if self.page is None else self.page.total_count} 份冻结报告"))
        controls.extend(rows or [ft.Text("暂无匹配报告")])
        ...
        if self.comparison:
            controls.append(ft.Text("报告比较", theme_style=ft.TextThemeStyle.TITLE_LARGE))
            ...
            controls.append(ft.Row([comparison], scroll=ft.ScrollMode.AUTO))
    ...
    def _detail_content(self):
        ...
        note = ft.TextField(label="评分备注（可选）", max_length=1000, expand=True)
