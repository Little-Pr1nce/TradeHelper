...
class ForecastEngine:
    ...
    def forecast(self, request: ForecastRequest, *, samples=(), industry_key: str | None = None) -> tuple[ForecastResult, ...]:
        ...
        for horizon in request.horizons:
            ...
            if baseline is not None:
                ...
                probabilities, distribution = predict_model(model, request.feature_snapshot)
                latest = self.registry.last_validation(
                    market=request.feature_snapshot.instrument.market,
                    scope_key=request.feature_snapshot.instrument.stable_key,
                    horizon=horizon,
                )
                validation_status, reason = latest or (ValidationStatus.NOT_EVALUATED, None)
                results.append(self._available(request, horizon, target, probabilities, distribution, scope, key, ModelFamily.EMPIRICAL, f"{scope.value}-empirical-baseline-h{horizon}", ModelLifecycle.CANDIDATE, validation_status, "tech", None, len(model.training_labels), 0, (), False, reason=reason))
            else:
    ...
    def _available(self, request, horizon, target, probabilities, distribution, scope, scope_key, family, version, lifecycle, status, feature_set_id, training_hash, sample_count, oof_count, drivers, execution_eligible, *, reason=None):
