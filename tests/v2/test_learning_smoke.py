...
from tradehelper_v2.contracts import (AdjustmentMode, CanonicalBar, DirectionProbabilities, ForecastAvailability, ForecastDirection, ForecastResult, ForecastScope, ModelFamily, ModelLifecycle, ValidationStatus)
...
def test_learning_migration_14_is_idempotent(tmp_path):
    ...
    try:
        assert repo._connection.execute('select max(version) from schema_migrations').fetchone()[0]==14
