...
from presentation_helpers import (
    forecast_outcome, research_validation_states, single_document, single_presentation,
)
from strategy_helpers import position
from tradehelper_v2.application.evaluation import HistoricalEvaluationService
...
from tradehelper_v2.presentation.renderers import render_html, render_markdown
...
def test_golden_aapl_forecast_states_execution_impact(us_instrument, now, calendar):
    ...
    assert table.columns[-1] == "执行影响" and all(row.cells[-1] for row in table.rows)
...
def test_golden_fcx_plan_has_reduce_and_reentry_branches(us_instrument, now, calendar):
    ...
    rows = _section(_document(instrument, now, calendar, **options), "plans").blocks[0].payload.rows
    assert any(row.cells[0] == "买入/加仓" and row.cells[1] == "add" for row in rows)
    assert any(row.cells[0] == "卖出/减仓" and row.cells[1] == "reduce" for row in rows)
...
def test_golden_glw_plan_preserves_invalidation(us_instrument, now, calendar):
    ...
    rows = _section(document, "plans").blocks[0].payload.rows
    assert any(row.cells[0] == "卖出/减仓" and row.cells[1] == "reduce" and row.cells[2] == "triggered" for row in rows)
...
def test_golden_wdc_history_does_not_override_current_risk(us_instrument, now, calendar):
    ...
    assert "sell" in _section(document, "plans").blocks[0].payload.rows[0].cells or any(row.cells[1] == "sell" for row in _section(document, "plans").blocks[0].payload.rows)
...
def test_golden_new_listing_sample_short_is_not_quality_failure(us_instrument, now, calendar):
    ...
    assert "模型/OOF" in table.columns and "数据质量" not in table.columns
...
def test_golden_no_take_profit_is_unquantifiable(us_instrument, now, calendar):
    rows = _section(_document(us_instrument, now, calendar), "plans").blocks[0].payload.rows
...
def test_golden_shared_trigger_profiles_explain_real_difference(us_instrument, now, calendar):
    table = _section(_document(us_instrument, now, calendar), "plans").blocks[0].payload
